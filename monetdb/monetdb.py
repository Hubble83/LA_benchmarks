...
def main(argv):
  ...
  MONET_DIR=os.environ["MONET_DIR"]
  DSTAT_DIR=os.environ["DSTAT_DIR"]

  with open("results_monetdb/query_{0}_time.csv".format(query), "a") as csv_time:
    csv_time.write(data_size)
  with open("results_monetdb/query_{0}_memory.csv".format(query), "a") as csv_memory:
  ...
  for i in range(2):
    ...
    with open("monetdb_current_query.sql", "w") as curr_query:
      curr_query.write(queries[query].format(data_size))

    execution = subprocess.Popen(
      [
        "{0}/monetdb/bin/mclient".format(MONET_DIR),
        "-t", "clock", "-d", "laolap",
        "monetdb_current_query.sql"
      ],
  ...
  for i in range(10):
    ...
    dstat = subprocess.Popen(
      [DSTAT_DIR+"/dstat", "-m", "--noheaders", "--noupdate", "--nocolor", "--output", "monetdb_memory_usage.csv"],
      stdout=subprocess.PIPE
    )
    time.sleep(2)
    ...
    os.remove("monetdb_current_query.sql")
    # get query results_monetdb
    ...
    # get profiler results_monetdb
    mem_out_list=[]
    with open("monetdb_memory_usage.csv", "r") as mem_info:
      mem_out_list = mem_info.readlines()[7:-1]
    os.remove("monetdb_memory_usage.csv")
    ...
    t_str = output_line_list[-1].split()[1]
