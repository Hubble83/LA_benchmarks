...
export DBGEN_DIR="$HOME/PI/tpch_2_17_0/dbgen"
...
mkdir -p "$BOOST_DIR" "$BISON_DIR" "$MYSQL_DIR" "DSTAT_DIR"
...
cd "$MYSQL_DIR/src" || return
git clone https://github.com/mysql/mysql-server.git
cd mysql-server || return
...
# Create MySQL configuration file (my.cnf)
{
	...
	echo -e "max_heap_table_size=220G"
	echo -e ""
	echo -e "query_cache_type = 1"
	echo -e "query_cache_size = 1M"
	echo -e ""
} > "$MY_CNF"
